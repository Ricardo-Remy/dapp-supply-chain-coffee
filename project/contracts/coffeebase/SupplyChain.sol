...
contract SupplyChain is DistributorRole, FarmerRole, ConsumerRole, RetailerRole, Ownable {
    ...
    modifier verifyCaller (address _address) {
        require(msg.sender == _address, "verifyCaller modifier");
        _;
    }

    // Define a modifier that checks if the paid amount is sufficient to cover the price
    modifier paidEnough(uint _price) {
        require(msg.value >= _price, "paidEnough modifier");
        _;
    }
    ...
    modifier harvested(uint _upc) {
        require(items[_upc].itemState == State.Harvested, "harvested modifier");
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Processed
    modifier processed(uint _upc) {
        require(items[_upc].itemState == State.Processed, "processed modifier");
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Packed
    modifier packed(uint _upc) {
        require(items[_upc].itemState == State.Packed, "packed modifier");
        _;
    }
    ...
    modifier sold(uint _upc) {
        require(items[_upc].itemState == State.Sold, "sold modifier");
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Shipped
    modifier shipped(uint _upc) {
        require(items[_upc].itemState == State.Shipped, "shipped modifier");
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Received
    modifier received(uint _upc) {
        require(items[_upc].itemState == State.Received, "received modifier");
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Purchased
    modifier purchased(uint _upc) {
        require(items[_upc].itemState == State.Purchased, "purchased modifier");
        _;
    }
    ...
    function kill() public onlyOwner {
        address payable payer = _make_payable(owner());
        selfdestruct(payer);
    }
